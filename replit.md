...
- Grew the pytest suite from 18 to 59 tests (added simulator scenario-branch coverage, demo-chat intent coverage, utils rendering tests, offline fallback coverage for every LLM chain, `app.py` module-routing tests, GroqClient exception/health-check paths, and `run_app.py` CLI-argument-filtering tests); added `pytest-cov` to `requirements.txt`. Coverage is 89% overall, 95% on `run_app.py`, 88% on the Groq client, 91%+ on the simulator, and 100% on schemas/memory.
- Efficiency: refactored `StadiumSimulator.get_stadium_context`/`get_crowd_context` to delegate to `@lru_cache`-wrapped pure module-level lookups keyed by scenario, avoiding rebuilding the same large literals on every Streamlit rerun. Public API unchanged; all tests still pass.
- Accessibility: added a screen-reader-only legend describing sidebar icon meanings and set `document.documentElement.lang = 'en'` via a tiny injected script, since Streamlit's default HTML shell doesn't set a document language.
